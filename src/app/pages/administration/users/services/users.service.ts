...
export class UsersService {
    ...
    search(filters: { identifiant?: string; firstName?: string; lastName?: string; email?: string; department?: string; }): Observable<AppUser[]> {
        ...
        const result = this.users.filter((u) => {
            ...
            const okDept = !department || u.department.toLowerCase().includes(department);
    ...
    exportCsv(rows: AppUser[]): string {
        const header = ['Unique Identifier', 'FirstName', 'LastName', 'Email', 'CIN', 'Phone', 'Active', 'Profession'];
